...
_main() {
  old_version="$(grep -o -E '^version\:[[:space:]]+([0-9.]+)$' src/netdata/*/Chart.yaml | sed -E -e 's/^version\:[[:space:]]+([0-9.]+)$/\1/')"
  old_appVersion="$(grep -o -E '^appVersion\:[[:space:]]+(v[0-9.]+)$' src/netdata/*/Chart.yaml | sed -E -e 's/^appVersion\:[[:space:]]+(v[0-9.]+)$/\1/')"
  ...
  git mv "src/netdata/$old_version" "src/netdata/$new_version"

  sed -i.bak \
    -e "s/$old_version/$new_version/g" \
    -e "s/$old_appVersion/v$new_appVersion/g" \
    "src/netdata/$new_version/README.md" \
    "src/netdata/$new_version/Chart.yaml" \
    "src/netdata/$new_version/values.yaml"
